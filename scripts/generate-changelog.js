...
async function main() {
  try {
    ...
    try {
      previousVersionTag = execSync('git describe --tags --abbrev=0 --match "v[0-9]*.[0-9]*.[0-9]*" --match "[0-9]*.[0-9]*.[0-9]*"').toString().trim();
      console.log(`Latest tag found: ${previousVersionTag}`);
    } catch (e) {
      try {
        previousVersionTag = execSync('git rev-list --max-parents=0 HEAD').toString().trim();
        console.log(`No version tags found. Using the first commit as the base: ${previousVersionTag}`);
    ...
    if (previousVersionTag) {
      commitLog = execSync(`git log ${previousVersionTag}..HEAD --pretty=format:"${commitLogFormat}"`).toString().trim();
    } else {
      commitLog = execSync(`git log --pretty=format:"${commitLogFormat}"`).toString().trim();
    }
    const commits = commitLog.split('\x00\n').filter(Boolean).map(commitEntry => {
      const parts = commitEntry.split('\x00');
      return { hash: parts[0], subject: parts[1], body: parts[2] || '' };
    });

    if (commits.length === 0 && !process.env.FORCE_UPDATE_CHANGELOG) { // Added a way to force update if needed
      console.log("No new commits found. Changelog will not be updated unless forced.");
      ...
      console.log(`Found ${commits.length} commits since ${previousVersionTag || 'the beginning'}.`);
    }
    ...
    commits.forEach(commit => {
      ...
      const match = commit.subject.match(/^([a-zA-Z]+)(\([a-zA-Z0-9_-]+\))?(!)?:(.*)/);
      if (match) {
      ...
      categorizedCommits[category].push(`- ${subject} ([${commit.hash.substring(0, 7)}])`);
    });
...
`;
    ...
    if (commits.length === 0) {
        ...
        for (const category in categorizedCommits) {
            newEntry += `### ${category}
`;
            categorizedCommits[category].forEach(commitMsg => {
                newEntry += `${commitMsg}
`;
            });
    ...
    if (unreleasedMatch && unreleasedMatch[1]) {
        existingUnreleasedContent = unreleasedMatch[1].trim() ? unreleasedMatch[1] : "\n"; // Preserve content or ensure a newline
    } else {
    ...
    if (!changelogContent.includes("## [")) { // If no other versions, ensure Unreleased is at the top
    ...
    if (insertPosition !== -1) {
        ...
        if (insertPosition !== -1) {
            ...
            insertPosition = headerBlockEnd !== -1 ? headerBlockEnd + 2 : changelogContent.length;
        } else {
    ...
    const topPart = changelogContent.match(/^# Changelog[\s\S]*?(?=\n## \[Unreleased\]|$)/);
    const mainHeaderPart = topPart ? topPart[0] : `# Changelog
...
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).`;
    ...
    const restOfChangelog = unreleasedMatch ? changelogContent.substring(unreleasedMatch.index + unreleasedMatch[0].length) : (changelogContent.includes(headerEndMarker) ? changelogContent.substring(changelogContent.indexOf(headerEndMarker) + headerEndMarker.length) : changelogContent );
    ...
    let unreleasedSectionContent = existingUnreleasedContent.trim() ? existingUnreleasedContent : "\n";
